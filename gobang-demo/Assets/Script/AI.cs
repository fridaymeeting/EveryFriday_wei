...
class AI
{
    ...
    public void CalcScore(int _mm, int _nn)
    {
        _cgrade = 0;
        _pgrade = 0;
        _board[_mm, _nn] = 1;//电脑下子位置     

        for (int i = 0; i < MaxFiveChainCount; i++)
        { 
            if (_ctable[_mm, _nn, i] && _win[0, i] != -1)
            {
                _win[0, i]++;//给白子的所有五连子可能的加载当前连子数  
            }

            if (_ptable[_mm, _nn, i])
            {
                _ptable[_mm, _nn, i] = false;
                _win[1, i] = -1;
            }
        }
    }
  ...
  public void setPlayerPiece( int playerX, int playerY )
    {
        int m = playerX;
        int n = playerY;
